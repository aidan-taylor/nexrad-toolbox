function convertedFiles = pyLevel2Archive(archiveFiles)
	...
	% =================
	%
	% archiveFiles (1,N) string
	%		Path to NEXRAD Level 2 archive files for conversion.
	%
	% =======
	% OUTPUTS
	% =======
	%
	% convertedFiles (1,N) string
	%		Path to converted matfiles containing the nexrad.core.Radar object.
	
	arguments
		archiveFiles (1,:) string = [];
	end
	...
	% Intialise output
	convertedFiles = strings(1, length(archiveFiles));
	
	for iFile = 1:length(archiveFiles)
		% Get current dataset path (TODO -- could be from cloud? [20/03/2025])
		sFile = archiveFiles(iFile);
		...
		convertedFiles(1, iFile) = finalFilename;
	end
