function matlabStruct = pyAwsNexradFile(pythonList)
	...
	% =================
	%
	% pythonList (1,N) py.list
	%		Python list containing AwsNexradFile objects generated by nexradaws
	%		module (nexrad.aws.*). These contain the metadata of archive files
	%		from the AWS bucket.
	%
	% =======
	% OUTPUTS
	% =======
	%
	% matlabStruct (1,1) struct
	%		Structure containing the metadata for each AwsNexradFile. Fields
	%		are: awspath, key, scan_time, radar_id, filename. 
